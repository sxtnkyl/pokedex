const typeColorConverter = props => {
  //gets props (a type name string, "fighting") finds the same type key in colorChart, returns a hex value {"type", "hex"}
  // console.log(props);
  ...
  const hex = [];
  props.forEach(prop =>
    hex.push({ ...colorChart.find(obj => obj.type === prop) })
  );
  return hex;
};

export default typeColorConverter;
